...
async function logAction(req, action, details, location) {
    ...
    } catch (err) { console.error('خطأ في تسجيل الحدث', err); }
}
...
app.put('/api/settings', requireAuth, requireAdmin, async (req, res) => {
    ...
    await pool.query(`UPDATE settings SET factories = $1, materials = $2, trucks = $3 WHERE id = 1`, [JSON.stringify(factories), JSON.stringify(materials), JSON.stringify(trucks)]);
    res.json({ success: true });
});
...
app.put('/api/day/:date', requireAuth, async (req, res) => {
    ...
    await pool.query(`INSERT INTO daily_data (date_key, orders, distribution, updated_at) VALUES ($1, $2, $3, CURRENT_TIMESTAMP) ON CONFLICT (date_key) DO UPDATE SET orders = $2, distribution = $3, updated_at = CURRENT_TIMESTAMP`, [date, JSON.stringify(orders || []), JSON.stringify(distribution || [])]);
    res.json({ success: true });
});
...
app.use((req, res) => { res.status(404).send('الصفحة غير موجودة 404'); });

// ==================== بدء التشغيل (الجزء الأهم) ====================
